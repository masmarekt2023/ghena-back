...
class adminController {
  ...
  // }
  async userList(req, res, next) {
    const validationSchema = Joi.object({  // Create schema object
        search: Joi.string().optional(),
        type: Joi.string().valid(userType.USER, userType.CREATOR).optional(),
        planType: Joi.string()
            .valid(
  ...
  async  listOrder(req, res, next) {
    try {
        ...
        const { value: validatedBody, error } = validationSchema.validate(req.query);
        if (error) {
        ...
        // Fetch order list using validated data
        const dataResults = await orderList({ status: { $ne: status.DELETE } }, validatedBody);
  ...
  async totalAdminBalance(req, res, next) {
    ...
    try {
        ...
        const address = adminResult.ethAccount.address;

        try {
            const [masBalance, bnbBalance, usdtBalance, fdusdBalance] = await Promise.all([
                bnb.mas.balance(address)
                    .catch(error => {
                        console.error('MAS balance error:', error);
                        return '0';
                    }),

                bnb.accountBalance(address)
                    .catch(error => {
                        console.error('BNB balance error:', error);
                        return '0';
                    }),

                bnb.usdt.balance(address)
                    .catch(error => {
                        console.error('USDT balance error:', error);
                        return '0';
                    }),

                bnb.fdusd.balance(address)
                    .catch(error => {
                        console.error('FDUSD balance error:', error);
                        return '0';
                    })
            ]);

            const obj = {
                masBalance: masBalance.toString(),
                bnbBalance: bnbBalance.toString(),
                usdtBalance: usdtBalance.toString(),
                fdusdBalance: fdusdBalance.toString()
            };
        ...
        console.error('Total admin balance error:', error);
        return next(error);
    }
}
  /**
  ...
  async  viewSocial(req, res, next) {
      ...
      try {
          // Validate request query
          const { value: validatedQuery, error } = validSchema.validate(req.query);
          if (error) {
              return res.status(400).json({
                  message: error.details[0].message,
                  status: 400,
          ...
          // Query the database for social records
          const find = await findSocial({
              title: title,
              status: { $ne: status.DELETE },
          });
          ...
          // Return the result
          return res.json({
              message: responseMessage.VIEW_SOCIAL,
              status: 200,
              data: find,
          });
      } catch (error) {
          // Log and handle errors
          console.error("Error in viewSocial:", error);
          return next(error);
      }
  }
  ...
  async  editSocial(req, res, next) {
      ...
      try {
          ...
          const { value: validatedBody, error } = validSchema.validate(req.body);
          if (error) {
              return res.status(400).json({
                  message: error.details[0].message,
                  status: 400,
          ...
          const { socialId, link } = validatedBody;
  
          // Verify the user's admin or sub-admin status
          ...
          // Find the social entry
          const data = await findSocial({
              _id: socialId,
              status: { $ne: status.DELETE },
          });
          if (!data) {
          ...
          const update = await updateSocialById(
              { _id: data._id },
              { $set: { link: link } }
          );
  ...
  async  transactionUsersList(req, res, next) {
    ...
    try {
      // Validate request query
      const { error, value: validatedBody } = validationSchema.validate(req.query, { abortEarly: false });
      if (error) {
        return res.status(400).json({ message: error.details.map((e) => e.message).join(', ') });
      }
  
      // Check user permissions
      const userResult = await allUser({
        _id: req.userId,
        userType: { $in: [userType.ADMIN, userType.SUB_ADMIN] },
      });
  
      if (!userResult) {
        return res.status(404).json({ message: responseMessage.USER_NOT_FOUND });
      }
  
      // Fetch transaction data
      const data = await allTransactions(validatedBody);
      return res.json(new response(data, responseMessage.DETAILS_FETCHED));
    } catch (error) {
      return next(error);
    }
  ...
  async  subAdminList(req, res, next) {
    ...
    try {
        ...
        const result = await listSubAdmin(validatedBody);
  ...
  async  getReferralSetting(req, res, next) {
    try {
        // Ensure the user exists and is authorized
        ...
        const result = await findReferral({ status: status.ACTIVE });
        ...
        console.error("Error in getReferralSetting:", error);
  ...
  async  updateReferralSetting(req, res, next) {
      ...
      try {
          // Validate request body
          const { value: validatedBody, error } = validationSchema.validate(req.body);
          if (error) {
              return res.status(400).json({
                  message: error.details[0].message,
                  status: 400,
          ...
          const { _id, referralAmount, refereeAmount } = validatedBody;
          ...
          // Log and handle errors
          console.error("Error in updateReferralSetting:", error);
          return next(error);
      }
  }
  ...
  async  listBanner(req, res, next) {
      ...
      try {
          ...
          const result = await paginateSearchBanner(validatedBody);
          if (!result) {
              return res.status(404).json({
                  message: responseMessage.DATA_NOT_FOUND,
                  status: 404,
          ...
          console.error("Error in listBanner:", error);
          return next(error);
      }
  }
